...
def get_two_different_vals(list_type):
    ...
    val1 = get_dummy_val_for(datatype) #rand no =2
    val2 = get_val_plus_delta(datatype, val1, 1) #rand no +1 =3
    return val1, val2
...
#attrib_types_dict = {'name_of_attribute': name, type: int/string}
...
class EquiJoin(WhereClause):
    ...
    def assign_value_to_list(self, list1, temp_copy, val1):
        for val in list1: #list1 = table,column 
            self.connectionHelper.execute_sql(
                ["update " + str(val[0]) + " set " + str(val[1]) + " = " + str(val1) + ";"]) #set all column values to 2
...
#here instead of negate we assign the value 2
    ...
    def get_join_graph(self, query):
        ...
        while global_key_lists:
            ...
            if len(join_keys) <= 1: #single node
